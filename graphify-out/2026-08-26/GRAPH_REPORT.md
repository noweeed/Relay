...
## Corpus Check
- 93 files · ~29,739 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 638 nodes · 1064 edges · 43 communities (25 shown, 18 thin omitted)
...
- `GroupedTasksResponse` --references--> `KanbanColumn`  [EXTRACTED]
  backend/src/services/task.service.ts → backend/src/models/Project.model.ts
...
Cohesion: 0.06
Nodes (31): dependencies, bcrypt, cookie-parser, cors, dotenv, express, express-rate-limit, helmet (+23 more)
...
Cohesion: 0.10
Nodes (34): TaskDocument, TaskPriority, TaskSource, TASK_ACTIVITY_ACTOR_TYPES, TASK_ACTIVITY_TYPES, TaskActivityActorType, TaskActivityDocument, taskActivitySchema (+26 more)

### Community 10 - "project.service.ts"
Cohesion: 0.06
Nodes (45): seed(), requireProjectRole(), firstColumnId(), KanbanMigrationResult, LegacyProjectRecord, migrateKanbanColumns(), migrateProject(), Membership (+37 more)
...
Cohesion: 0.07
Nodes (35): deleteProject(), getProject(), getProjectContext(), inviteProjectMember(), listProjectMembers(), removeProjectMember(), updateProject(), requireProjectMembership() (+27 more)

### Community 39 - "auth.service.ts"
Cohesion: 0.11
Nodes (33): getRefreshCookieOptions(), getSessionMetadata(), login(), logout(), me(), refresh(), sendAuthenticationResult(), signup() (+25 more)

### Community 40 - "check-ai.mjs"
Cohesion: 0.50
Nodes (4): aiServiceDirectory, backendDirectory, main(), resolvePythonInterpreter()

### Community 41 - "task.controller.ts"
Cohesion: 0.15
Nodes (20): createTask(), deleteTask(), getTask(), getTaskContext(), getTaskId(), listTaskActivity(), listTasks(), updateTask() (+12 more)

### Community 42 - "meeting.service.ts"
Cohesion: 0.07
Nodes (42): createMeeting(), getMeeting(), getMeetingContext(), getMeetingId(), getMeetingTasks(), getMeetingTranscript(), listMeetings(), requestReprocess() (+34 more)

## Knowledge Gaps
- **216 isolated node(s):** `relay-ai-service`, `$schema`, `$id`, `title`, `type` (+211 more)
...
- **Why does `ApiError` connect `project.service.ts` to `app.ts`, `auth.service.ts`, `task.controller.ts`, `meeting.service.ts`, `task.service.ts`, `kanban.service.ts`, `project.routes.ts`?**
