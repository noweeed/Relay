...
function getProjectContext(request: Request): {
  ...
  if (!request.user || !request.projectMembership) {
    throw new ApiError(
      500,
      "INTERNAL_ERROR",
      "Project authorization context is missing.",
    );
  }
...
/** Edits a member's descriptive team role without changing access permissions. */
export async function updateProjectMember(
  ...
  const member = await projectService.updateProjectMember(
    context.projectId,
    targetUserId,
    request.body as UpdateProjectMemberInput,
  );
  response.json({ success: true, data: member });
}

/** Transfers project ownership to a selected existing member. */
export async function transferProjectOwnership(
  ...
  await projectService.transferProjectOwnership(
    context.projectId,
    context.userId,
    request.body as TransferProjectOwnershipInput,
  );
  response.json({ success: true, data: { transferred: true } });
}
