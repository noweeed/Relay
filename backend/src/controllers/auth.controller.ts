...
export async function refresh(
  ...
  const currentToken = request.cookies[env.REFRESH_COOKIE_NAME] as
    string | undefined;
  if (!currentToken)
    throw new ApiError(401, "UNAUTHORIZED", "A refresh token is required.");
...
/** Updates the authenticated user's name or profile image. */
...
/** Changes a local password and replaces all old refresh sessions. */
...
/** Replaces the authenticated user's complete notification preference set. */
export async function updateNotificationPreferences(
  ...
  const user = await authService.updateNotificationPreferences(
    request.user.id,
    request.body as NotificationPreferencesInput,
  );
  response.json({ success: true, data: user });
}

/** Permanently deletes an eligible account and clears its refresh cookie. */
...
/** Exchanges a verified Google credential for Relay access and refresh tokens. */
export async function googleAuthentication(
  request: Request,
  response: Response,
): Promise<void> {
  const result = await authService.authenticateWithGoogle(
    request.body as GoogleAuthenticationInput,
    getSessionMetadata(request),
  );
  sendAuthenticationResult(response, result, 200);
}
